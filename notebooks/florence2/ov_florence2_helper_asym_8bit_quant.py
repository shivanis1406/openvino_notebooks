...
from nncf import compress_weights  # Import NNCF weight compression
...
def compress_and_save(model, output_path):
    """
    Compress weights of the model using NNCF and save in OpenVINO IR format.
    """
    # Asymmetrical 8-bit quantization of the model weights represented in OpenVINO IR using NNCF : https://docs.openvino.ai/2024/openvino-workflow/model-optimization-guide/weight-compression.html
    compressed_model = compress_weights(model)
    
    # Save the compressed model
    ov.serialize(compressed_model, str(output_path.with_suffix('.xml')), str(output_path.with_suffix('.bin')))
    print(f"✅ Compressed model saved to: {output_path.with_suffix('.xml')}")
...
def convert_florence2(model_id, output_dir):
    ...
    if not (output_dir / IMAGE_EMBEDDING_NAME).exists():
        ...
        compress_and_save(ov_model, output_dir / IMAGE_EMBEDDING_NAME)
        del ov_model
    ...
    if not (output_dir / TEXT_EMBEDING_NAME).exists():
        ...
        #Compress and save the model
        compress_and_save(ov_model, output_dir / TEXT_EMBEDING_NAME)
        del ov_model
        cleanup_torchscript_cache()
        gc.collect()
        print("✅ Text Embedding conversion started")
    ...
    if not (output_dir / ENCODER_NAME).exists():
        ...
        compress_and_save(ov_model, output_dir / ENCODER_NAME)
        del ov_model
        cleanup_torchscript_cache()
        gc.collect()
    ...
    if not (output_dir / DECODER_NAME).exists() or not (output_dir / DECODER_WITH_PAST_NAME).exists():
        ...
        if not (output_dir / DECODER_NAME).exists():
            ...
            compress_and_save(ov_model, output_dir / DECODER_NAME)
            del ov_model
            cleanup_torchscript_cache()
            gc.collect()
        ...
        if not (output_dir / DECODER_WITH_PAST_NAME).exists():
            ...
            compress_and_save(ov_model, output_dir / DECODER_WITH_PAST_NAME)
            del ov_model
            cleanup_torchscript_cache()
            gc.collect()
